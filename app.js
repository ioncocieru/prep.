...
/* Sigiliul (culoare + iconiță) fiecărui examen — folosit pe biletele din dashboard */
const EXAM_SEALS = {
  python: {
    color: "#2E4A78",
  ...
  networking: {
    color: "#A85A26",
    icon: `<circle cx="5" cy="12" r="2"/><circle cx="19" cy="6" r="2"/><circle cx="19" cy="18" r="2"/><path d="M7 12h6M13 12l4-4M13 12l4 4"/>`
  }
};
...
/* =====================================================================
   4) DASHBOARD — alegerea examenului (bilete de certificare)
   ===================================================================== */
function renderDashboard() {
  ...
  EXAM_ORDER.forEach(key => {
    ...
    const seal = EXAM_SEALS[key] || { color: "#1F4B3F", icon: "" };

    const card = document.createElement("div");
    card.className = "ticket-card";
    card.innerHTML = `
      <div class="ticket-top">
        <span class="ticket-seal" style="--seal:${seal.color}; --seal-tint:color-mix(in srgb, ${seal.color} 12%, white);">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">${seal.icon}</svg>
        </span>
        <div>
          <h3>${exam.name}</h3>
        </div>
      ...
      <div class="ticket-stub">
        <div class="ticket-stub-meta">
          <strong>${total} întrebări</strong>
          <span>${chapterCount} capitole disponibile</span>
        </div>
...
/* ---------- Render: Tabel (conținut atașabil oricărui tip de întrebare) ---------- */
function renderTable(table) {
  if (!table || !Array.isArray(table.headers) || !Array.isArray(table.rows)) return "";
  let html = `<div class="q-table-wrap">`;
  if (table.caption) html += `<div class="q-table-caption">${escapeHtml(table.caption)}</div>`;
  html += `<table class="q-table"><thead><tr>`;
  table.headers.forEach(h => { html += `<th>${escapeHtml(String(h))}</th>`; });
  html += `</tr></thead><tbody>`;
...
function finishQuiz() {
  ...
  $("#results-sub").textContent = `${state.quizMode === "practice" ? "Mod exercițiu" : "Mod examen"}, ${total} întrebări`;
  ...
  $("#score-pass-label").style.color = passed ? "var(--leaf)" : "var(--clay)";

  const ring = $("#score-ring-fill");
  const circumference = 389.6;
  ring.style.stroke = passed ? "var(--leaf)" : "var(--gold)";
